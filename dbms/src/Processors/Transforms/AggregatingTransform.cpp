...
namespace
{
    ...
    private:
        ...
        void initialize()
        {
            ...
            AggregatedDataVariantsPtr & first = data->variants[0];

            if (first->type == AggregatedDataVariants::Type::without_key || params->params.overflow_row)
            {
                params->aggregator.mergeWithoutKeyDataImpl(data->variants);
        ...
        #define M(NAME) \
                        ...
                        params->aggregator.mergeSingleLevelDataImpl<decltype(first->NAME)::element_type>(data->variants);
            if (false) {}
...
void AggregatingTransform::initGenerate()
{
    ...
    if (!params->aggregator.hasTemporaryFiles())
    {
        processors.emplace_back(std::make_shared<ConvertingAggregatedToChunksTransform>(params, many_data, max_threads));
    }
    else
    {
